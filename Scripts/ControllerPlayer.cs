...
public class ControllerPlayer : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!PauseMenu.GameIsPaused && !DetectCollision.dead)
        {
            ...
            if (/*Input.GetMouseButtonDown(0)*/ Input.GetKeyDown("space") && charger.Count <= 1000)
            {
                audioData.Play(0);
            } else if (/*Input.GetMouseButtonDown(0) */Input.GetKeyDown("space") && charger.Count >= 1000)
            {
                audioCharger.PlayOneShot(noHammo, 1f);
            }
            if (/*Input.GetMouseButton(0)*/ Input.GetKey("space") && charger.Count <= 1000 )
            {
            ...
            if (/*Input.GetMouseButtonUp(0) */Input.GetKeyUp("space") || charger.Count > 1000)
            {
